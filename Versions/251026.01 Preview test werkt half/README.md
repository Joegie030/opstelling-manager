...
│       │   └── kwarten/{kwartNum}
│       │       ├── nummer: number (1-4)
│       │       ├── opstelling: { [positie]: spelerIdStr }
│       │       ├── wissels: Wissel[]
│       │       ├── minuten: number
...
│       │
│       └── statistieken/
│           ├── doelpuntenMakers: { [spelerId]: aantal }
│           ├── speelminuten: { [spelerId]: aantal }
│           ├── keeperBeurten: { [spelerId]: aantal }
│           └── teamStats: { won, lost, draw, doelsaldo }
│
├── coaches/{uid}
│   ├── uid: string
│   ├── email: string
...
- ✅ Imports bijgewerkt
- ✅ Database v2 structuur geimplementeerd (seizoenen support)

### 🔄 Phase 3: TSX Validation (ACTUEEL)
- 🔄 Alle TSX bestanden controleren op database v2 compatibility
- 🔄 seizoenId parameter overal toevoegen waar nodig
- 🔄 Real-time listeners updaten (onWedstrijdenChange, etc.)
- 🔄 State management voor seizoen selectie

### 📋 Phase 4: Custom Hooks (Toekomst)
...
### 🎯 Phase 5: Componenten Splitsen (Later)
...
│   ├── WedstrijdOverzicht.tsx   # Wedstrijdenlijst (per seizoen)
│   ├── WedstrijdOpstelling.tsx  # Opstelling editor (Phase: kan gesplitst worden)
│   ├── Statistieken.tsx         # Statistieken overzicht (per seizoen)
│   ├── TeamBeheer.tsx           # Spelers beheer
│   ├── SeizoenenBeheer.tsx      # Seizoen management
...
│
├── types/
│   └── index.ts                 # Type definitions (Speler, Wedstrijd, Seizoen, etc)
│
├── firebase/
│   └── firebaseService.ts       # Firebase auth & Firestore (v2 structuur)
│
...
## 📋 API Reference - Firebase Service Functions
...
- `inviteCoach(teamId, email, invitedByUid)` - Send invite
- `getPendingInvites(email)` - Get pending invitations
- `acceptInvite(inviteId, userUid, teamId)` - Accept invitation
...
Versie: 3.0 (Oktober 2025 - Database v2)
**Status:** Phase 3 - TSX Validation in progress
