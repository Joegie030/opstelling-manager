...
const App = () => {
  ...
  const renderStatistieken = () => {
    ...
    return (
      <div className="space-y-6">
        <h2 className="text-2xl font-bold">Positie Statistieken</h2>
        {wedstrijden.length === 0 ? (
          <p className="text-gray-500 text-center py-8">Nog geen wedstrijden gespeeld</p>
        ) : (
          <div className="border rounded-lg p-4 bg-white">
            <h3 className="text-xl font-semibold mb-3">Hoe vaak heeft elke speler op elke positie gespeeld?</h3>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-2 px-2 sticky left-0 bg-white">Speler</th>
                    {posities.map(p => (
                ...
                <tbody>
                  {stats.sort((a, b) => a.naam.localeCompare(b.naam)).map(spelerStat => {
                    const totaal = Object.values(spelerStat.posities).reduce((sum, val) => sum + val, 0);
                    return (
                      <tr key={spelerStat.naam} className="border-b hover:bg-gray-50">
                        <td className="py-2 px-2 font-medium sticky left-0 bg-white">{spelerStat.naam}</td>
